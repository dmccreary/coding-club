...
writing-style rules, the MicroSim expectations, the Markdown formatting
rules, and the learning mascot (Circuit the Robot) placement rules for this
book. Content that ignores it produces broken layouts and an inconsistent
student experience. Note that the teacher guide, instructor guide, or other
instructor-facing content does not use the mascot described in
`CONTENT-GENERATION-GUIDE.md`.
...
| | |
|---|---|
| Book title | Coding Club |
| Published site | https://dmccreary.github.io/coding-club/ |
| Repository | https://github.com/dmccreary/coding-club |
...
**Model:** This project's default model is Sonnet (latest), set in
`.claude/settings.json`. Before generating or substantially editing a MicroSim
— running the `microsim-generator`, `microsim-p5`, or `breadboard-sim-generator`
skill, or hand-writing sim JS/HTML — switch to Opus (latest) with `/model opus`.
Switch back to Sonnet with `/model sonnet` once the sim work is done. Opus
reasons more reliably through the interactive layout and state logic these
sims require; Sonnet is the default for everything else in this repo.
...
- The published lesson URL is
  `https://dmccreary.github.io/coding-club/sims/<sim-id>/`. Always
